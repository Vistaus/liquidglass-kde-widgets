...
// Analog-clock variant of the shared appearance config.
//
// Differs from 1-common/config/ConfigAppearance.qml only in the addition of
// `opaqueBackground`: in Solid mode the analog clocks render their background
// squircle with the glass material by default (solid clock face on a glass
// card). Checking "Fully opaque background" reverts to the flat opaque fill.
//
// Because the glass material now also backs Solid mode (unless opaque), the
// "Glass effect" section is shown whenever the glass backdrop is actually in
// use — i.e. Glass style, or Solid style with a non-opaque background.
ColumnLayout {
    id: root
    ...
    property alias cfg_opaqueBackground: opaqueCheck.checked

    // Glass backdrop is live in Glass style, or Solid style with a
    // non-opaque background.
    readonly property bool glassBackdropActive:
        styleCombo.currentIndex === 0 || !opaqueCheck.checked
    ...
    function _deserialize(text) {
        try {
            ...
            if (o.ob !== undefined) opaqueCheck.checked           = o.ob
    ...
    Kirigami.FormLayout {
        ...
        CheckBox {
            ...
            text: i18n("Fully opaque background")
    ...
    Kirigami.FormLayout {
        id: glassSection
        Layout.fillWidth: true
        visible: root.glassBackdropActive
